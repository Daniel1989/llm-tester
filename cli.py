...
from exceptions import TooDeepRecursionError
from exceptions import TokenLimitError
from exceptions import CommandFinishedEarly
...
def terminate_process(pid: int, name=None) -> None:
    ...
    try:
        ...
        print("Timeout expired while waiting for process to terminate.")
...
def execute_command(project, command, timeout=None, success_message=None, command_id: str = None, force=False) \
    ...
    if not force:
        question = f'Can I execute the command: `{command}`'
        if timeout is not None:
...
def check_if_command_successful(convo, command, cli_response, response, exit_code, additional_message=None,
    ...
    if cli_response is not None:
        if exit_code is None:
            ...
            print(f'Process for command {command} still running.')
            response = 'DONE'
        else:
